 ...
 *
 * Usage:
 *   node scripts/import-substack.js <path-to-substack-export-folder>
 *
 * Point it at the top-level export folder (the one containing posts/ and posts.csv).
 * It reads posts.csv for titles, subtitles, and dates, then converts each HTML post.
 */
...
try {
  ...
  console.error('\nMissing dependency. Run: npm install cheerio\n');
  process.exit(1);
}
...
function displayDate(str) {
  ...
  } catch { return 'June 2026'; }
}

// ── Parse posts.csv ───────────────────────────────────────────────────────────
// Columns: post_id, post_date, is_published, email_sent_at, inbox_sent_at,
...
function loadPostsMeta(csvPath) {
  const lines = fs.readFileSync(csvPath, 'utf8').split('\n').filter(Boolean);
  const meta  = {};
  for (let i = 1; i < lines.length; i++) {        // skip header
    const cols = parseCSVLine(lines[i]);
    if (cols.length < 8) continue;
    const postId   = cols[0].trim();               // e.g. "182864584.cycling-for-beginners…"
    const numId    = postId.split('.')[0];          // just the numeric part
    meta[postId]   = { date: cols[1], title: cols[7], subtitle: cols[8] || '' };
    meta[numId]    = meta[postId];                  // also index by number alone
  }
  return meta;
}

// ── Parse a Substack export HTML file ────────────────────────────────────────
// The export is raw post content — no <html>/<body> wrapper.
// The entire file IS the article body.

function parsePost(html, fallbackTitle) {
  const $ = cheerio.load(html);

  // Extract title from first non-empty <h1>
  let title = '';
  $('h1').each((_, el) => {
    const t = $(el).text().trim();
    if (t && !title) title = t;
  });
  if (!title) title = fallbackTitle || 'Untitled';

  // Remove elements we don't want in the output
  $('h1').each((_, el) => {
    // remove empty h1s and the title h1 (we render our own styled title)
    if (!$(el).text().trim() || $(el).text().trim() === title) $(el).remove();
  });

  // Remove Substack UI chrome
  ...
  // Remove the expand/restack button overlays on images (keep the actual img)
  $('.image-link-expand').remove();
  // Remove pencraft UI elements (share/restack icons)
  $('[class^="pencraft"]').remove();
...
<head>
    ...
    <style>
        ...
        .post-body ol { padding-left:1.75rem; margin-bottom:1.25rem; }
        ...
        .post-body img { max-width:100%; border-radius:.75rem; margin:1.5rem auto; display:block; border:1px solid rgba(255,255,255,.05); }
        .post-body figure { margin:1.5rem 0; }
        .post-body figcaption { color:#6b7280; font-size:.85rem; text-align:center; margin-top:.5rem; }
        .post-body .captioned-image-container { margin:1.5rem 0; }
        .post-body .captioned-image-container a { text-decoration:none; display:block; }
        .post-body .captioned-image-container picture { display:block; }
        .post-body hr { border:none; border-top:1px solid rgba(255,128,0,.15); margin:2.5rem 0; }
        .post-body iframe { width:100%; border-radius:.75rem; margin:1.5rem 0; }
        .post-body .youtube-wrap { margin:1.5rem 0; }
        .post-body .youtube-inner { position:relative; padding-bottom:56.25%; height:0; overflow:hidden; border-radius:.75rem; }
        .post-body .youtube-inner iframe { position:absolute; top:0; left:0; width:100%; height:100%; border:none; }
    </style>
...
// ── Blog card for blog.html ───────────────────────────────────────────────────

function buildBlogCard({ title, slug, excerpt, dateRaw, category }) {
  const dispDate = displayDate(dateRaw);
  const titleEsc = escHtml(title);
  const excerptEsc = escHtml(excerpt).substring(0, 160);
  return `
                <!-- Post: ${titleEsc} -->
...
function updateBlogIndex(newCards, totalPosts) {
  ...
  if (!html.includes('<!-- IMPORT_END -->')) {
    console.warn('\n  [WARN] blog.html missing <!-- IMPORT_END --> marker.');
    console.log('  Add these cards manually to the grid in blog.html:\n');
    newCards.forEach(c => console.log(c));
    return;
  }

  html = html.replace('<!-- IMPORT_END -->', newCards.join('\n') + '\n            <!-- IMPORT_END -->');
  html = html.replace(/\d+ Posts? Published/, `${totalPosts} Post${totalPosts !== 1 ? 's' : ''} Published`);
  fs.writeFileSync(blogPath, html);
  console.log(`  blog.html updated (${totalPosts} posts total)`);
}
...
if (!exportDir) {
  console.log('\nUsage:  node scripts/import-substack.js <substack-export-folder>');
  console.log('Example: node scripts/import-substack.js ~/Downloads/substack_export\n');
  process.exit(1);
}
...
if (!fs.existsSync(postsDir)) {
  console.error(`\nCould not find posts/ folder inside: ${exportDir}`);
  process.exit(1);
}

// Load metadata from posts.csv
const meta = fs.existsSync(csvPath) ? loadPostsMeta(csvPath) : {};
if (!Object.keys(meta).length) {
  console.warn('  [WARN] posts.csv not found — titles and dates will be extracted from HTML only');
}
...
console.log(`\nFound ${htmlFiles.length} HTML post(s) in ${postsDir}\n`);
...
for (const file of htmlFiles) {
  // Filename: "182864584.cycling-for-beginners-over-50-5-steps.html"
  const basename = path.basename(file, '.html');          // "182864584.cycling-for-..."
  const numId    = basename.split('.')[0];                 // "182864584"
  const fileSlug = basename.slice(numId.length + 1);      // "cycling-for-..."

  // Look up metadata from CSV (match by full basename or numeric ID)
  const csvMeta  = meta[basename] || meta[numId] || {};
  const title    = csvMeta.title  || '';
  const subtitle = csvMeta.subtitle || '';
  const dateRaw  = csvMeta.date   || new Date().toISOString();

  // Parse HTML body
  const html     = fs.readFileSync(path.join(postsDir, file), 'utf8');
  const parsed   = parsePost(html, title || fileSlug.replace(/-/g, ' '));
  const finalTitle = csvMeta.title || parsed.title;
  const slug     = fileSlug || slugify(finalTitle);
  ...
  // Build blog card
  const excerpt = (subtitle || parsed.bodyText.replace(/\s+/g, ' ').substring(0, 150)).trim() + '…';
  cards.push(buildBlogCard({ title: finalTitle, slug, excerpt, dateRaw, category }));
  imported.push({ title: finalTitle, slug, dateRaw });

  const rt = readTime(parsed.bodyText);
  console.log(`  [OK]  Blog/${slug}.html  "${finalTitle}"  (${rt})`);
}

if (imported.length) {
  console.log('');
  const totalPosts = fs.readdirSync(BLOG_DIR).filter(f => f.endsWith('.html')).length;
  updateBlogIndex(cards, totalPosts);
  updateSitemap(imported);
  console.log(`\nDone! Imported ${imported.length} post(s), skipped ${skipped}.`);
  console.log('\nNext steps:');
  console.log('  1. Review the pages in Blog/ — check images and formatting');
  console.log('  2. git add Blog/ blog.html sitemap.xml && git commit -m "feat: import Substack posts" && git push');
} else {
  console.log(`\nNothing new to import (${skipped} already existed).`);
}
