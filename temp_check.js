...
<body class="bg-black text-gray-200 font-body antialiased flex h-screen overflow-hidden">
    <style>
        .kanban-col {
            min-height: 500px;
        }
    ...
    <!-- SIDEBAR -->
    <aside class="w-64 bg-[#050511] border-r border-gray-800 flex flex-col flex-shrink-0 z-40">
        <!-- Brand -->
    ...
    <!-- MAIN CONTENT -->
    <main class="flex-1 relative overflow-hidden bg-black h-full">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto h-full">
                ...
                <div class="lg:col-span-2 flex flex-col gap-6 h-full">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 h-64">
                        <!-- Pie Chart -->
                        <div class="bg-[#0B0B2B] border border-gray-800 rounded-xl p-4 flex flex-col">
                            <h3 class="font-display font-bold text-lg text-white uppercase mb-2">Job Mix</h3>
                        ...
                        <div class="bg-[#0B0B2B] border border-gray-800 rounded-xl p-6 relative">
                            ...
                            <div class="text-left">
                                <h4 class="font-bold text-brand-orange uppercase text-xs tracking-wider mb-1">Hours of
                                    Operation</h4>
                                <p id="shop-hours" class="text-gray-400 text-sm whitespace-pre-line mb-4">Mon-Fri: 9am -
                                    5pm\nSat: 10am - 2pm\nSun: Closed</p>
        ...
        <!-- CONTACTS VIEW -->
        <div id="view-contacts" class="hidden absolute inset-0 p-8 overflow-auto bg-brand-dark">
            <div class="max-w-7xl mx-auto">
                <div class="flex justify-between items-center mb-8">
                    <h2 class="font-display font-bold text-4xl text-white uppercase">Contacts</h2>
                    <div class="relative">
                        <i
                        ...
                        <input type="text" id="contact-search" placeholder="Search customers..."
                            ...
                            class="bg-[#050511] border border-gray-700 text-white pl-10 pr-4 py-2 rounded w-64 focus:border-brand-orange focus:outline-none">
                    </div>
                    <button onclick="startWalkInJob()"
                        class="bg-brand-orange hover:bg-orange-600 text-white font-bold uppercase tracking-wider px-4 py-2 rounded transition flex items-center gap-2 ml-4">
                        <i class="fa-solid fa-plus-circle"></i> New Job
                    </button>
                </div>

                <div class="bg-[#0B0B2B] border border-gray-800 rounded-xl overflow-hidden">
    ...
    <dialog id="new-job-modal"
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <select id="new-job-service"
                    class="w-full bg-[#0B0B2B] text-white rounded border border-gray-700 p-2 focus:border-green-500 focus:outline-none">
                    <option value="Safety Check">Safety Check ($50)</option>
                    <option value="The Tune-Up">The Tune-Up ($90)</option>
                    <option value="Overhaul">Overhaul ($180)</option>
                    <option value="Dream Build">Dream Build ($300)</option>
                    <option value="A La Carte">A La Carte (Varies)</option>
                </select>
            </div>
        </div>
    ...
    <dialog id="job-sheet-modal"
        ...
        <div class="flex h-[80vh] flex-col">
            ...
            <!-- Body -->
            <div id="modal-checklist" class="flex-grow overflow-y-auto p-8 space-y-8">
                ...
                async function renderContacts() {
                    ...
                    contacts.filter(c => (c.name || '').toLowerCase().includes(search) || (c.email || '').toLowerCase().includes(search))
                        ...
                        <td class="p-4 text-right flex justify-end gap-2">
                             <!-- Buttons visible always for clarity, or on group hover -->
                            <button onclick="startNewJob('${c.id}')" title="New Job" class="text-green-500 hover:text-white bg-gray-800 hover:bg-green-600 p-2 rounded transition"><i class="fa-solid fa-plus"></i></button>
                            <button onclick="viewContactHistory('${c.id}', '${c.name.replace(/'/g, "\\'")}')" title="Job History" class="text-brand-orange hover:text-white bg-gray-800 hover:bg-brand-orange p-2 rounded transition"><i class="fa-solid fa-clock-rotate-left"></i></button>
                            <button onclick="editContact('${c.id}')" title="Edit Contact" class="text-gray-400 hover:text-brand-orange bg-gray-800 hover:bg-white p-2 rounded transition"><i class="fa-solid fa-pen-to-square"></i></button>
